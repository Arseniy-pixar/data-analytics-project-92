...
select concat(c.first_name, ' ', c.last_name) as seller, -- формируем название продавца (имя + фамилия)
...
group by c.first_name, c.last_name, o.avg_income -- группируем по продавцам и по общей средней выручке
...
ЗАПРОС 3
select concat(c.first_name,' ', c.last_name) as seller, -- формирует столбец 'seller' с именем и фамилией продавца
